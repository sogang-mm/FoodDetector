...
def test(model, annotations, cfg):
    ...
    (width,height)=(model.width,model.height) if not hasattr(model,'module') else (model.module.width,model.module.height)
    print(width,height)

    # do one forward pass first to circumvent cold start
    throwaway_image = Image.open('data/dog.jpg').convert('RGB')
    throwaway_image_size=throwaway_image.size
    throwaway_image=np.array(throwaway_image.resize((width, height)))
    # do_detect(model, throwaway_image, 0.5, 80, 0.4, use_cuda)
    print(type(throwaway_image))
    print(throwaway_image.shape)
    throwaway_image_outputs=do_detect(model, throwaway_image, 0.5, 0.4, use_cuda)
    print(throwaway_image_outputs)
    throwaway_image_bbox=[]
    for box in throwaway_image_outputs[0]:
        box_json = {}
        category_id = box[-1]
        score = box[-2]
        ...
        for i, bbox_coord in enumerate(bbox_normalized):
            ...
            if i % 2:
                modified_bbox_coord *= throwaway_image_size[1]
            else:
                modified_bbox_coord *= throwaway_image_size[0]
            modified_bbox_coord = round(modified_bbox_coord, 2)
            throwaway_image_bbox.append(modified_bbox_coord)
    print(throwaway_image_bbox)
    ...
    for i, image_annotation in enumerate(images):
        ...
        sized = np.array(img.resize((width,height)))
...
def get_args(**kwargs):
    ...
    parser.add_argument('-g', '--gpu', metavar='G', type=str, default='0,1',
                        help='GPU', dest='gpu')
    parser.add_argument('-dir', '--data-dir', type=str, default='/nfs_shared/mscoco/val2017',
                        help='dataset dir', dest='dataset_dir')
    parser.add_argument('-gta', '--ground_truth_annotations', type=str, default='/nfs_shared/mscoco/annotations/instances_val2017.json',
