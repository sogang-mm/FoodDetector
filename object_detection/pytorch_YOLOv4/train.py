...
def train(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', ncols=200) as pbar:
...
def evaluate(model, data_loader, cfg, device, logger=None, **kwargs):
    ...
    with tqdm(total=len(data_loader.dataset),ncols=200,unit='img') as bar:
        for images, targets in data_loader:
            ...
            for img, target, boxes, confs in zip(images, targets, outputs[0],outputs[1]):
                ...
                boxes = boxes.squeeze().cpu().detach().numpy()
            ...
            bar.update(model_input.shape[0])
...
def get_args(**kwargs):
    ...
    parser.add_argument('-classes', type=int, default=150, help='dataset classes')
    parser.add_argument('-train_label_path', dest='train_label', type=str, default='/workspace/object_detection/pytorch_YOLOv4/data/train_od.txt', help="train label path")
    parser.add_argument(
